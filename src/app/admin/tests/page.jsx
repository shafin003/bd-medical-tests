...
export default function AdminTestsPage() {
	...
	const handleBulkPriceChange = (serviceId, field, value) => {
		setBulkPriceUpdates((prevUpdates) =>
			prevUpdates.map((service) => (service.id === serviceId ? { ...service, [field]: value } : service))
		);
	};
	...
	const handleCategoryDelete = async (id) => {
		if (!confirm('Are you sure you want to delete this category? This will also affect tests linked to it.'))
			return;
					...
					<Dialog open={isTestDialogOpen} onOpenChange={setIsTestDialogOpen}>
						...
						<DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
							<DialogHeader>
								...
								<DialogDescription>
									{currentTest
										? 'Edit the details of the medical test.'
										: 'Fill in the details to add a new medical test.'}
								</DialogDescription>
							</DialogHeader>
							<form onSubmit={handleTestSubmit} className="grid gap-4 py-4">
								...
								<div className="grid grid-cols-4 items-center gap-4">
									<Label htmlFor="testCategoryId" className="text-right">
										Category
									</Label>
								...
								<div className="grid grid-cols-4 items-center gap-4">
									<Label htmlFor="testPurpose" className="text-right">
										Purpose
									</Label>
									<Textarea
										id="testPurpose"
								...
								<div className="grid grid-cols-4 items-center gap-4">
									<Label htmlFor="fastingRequired" className="text-right">
										Fasting Required
									</Label>
									<Switch
										id="fastingRequired"
										checked={fastingRequired}
										onCheckedChange={setFastingRequired}
										className="col-span-3"
									/>
								</div>
								<div className="grid grid-cols-4 items-center gap-4">
								...
								<div className="grid grid-cols-4 items-center gap-4">
									<Label htmlFor="commonSymptoms" className="text-right">
								...
								<div className="grid grid-cols-4 items-center gap-4">
									<Label htmlFor="ageRestrictions" className="text-right">
										Age Restrictions
									</Label>
									<Input
										id="ageRestrictions"
								...
								<div className="grid grid-cols-4 items-center gap-4">
									<Label htmlFor="genderSpecific" className="text-right">
										Gender Specific
									</Label>
				...
				<CardHeader className="flex flex-row justify-between items-center">
					...
					<Dialog open={isCategoryDialogOpen} onOpenChange={setIsCategoryDialogOpen}>
						...
						<DialogContent className="sm:max-w-[425px]">
							<DialogHeader>
								<DialogTitle>
									{currentCategory ? 'Edit Test Category' : 'Add New Test Category'}
								</DialogTitle>
								<DialogDescription>
									{currentCategory
										? 'Edit the details of the test category.'
										: 'Fill in the details to add a new test category.'}
								</DialogDescription>
							</DialogHeader>
			...
			<Card className="mb-8">
				...
				<CardContent>
					<div className="grid gap-4">
						<div className="grid grid-cols-4 items-center gap-4">
							<Label htmlFor="selectTestForBulkUpdate" className="text-right">
								Select Test
							</Label>
							<Select
								onValueChange={handleSelectTestForBulkUpdate}
								value={selectedTestForBulkUpdate || ''}
							>
								<SelectTrigger className="col-span-3">
						...
						{bulkPriceUpdates.length > 0 && (
							<div className="overflow-x-auto">
								<h3 className="text-lg font-semibold mb-2">
									Prices for {tests.find((t) => t.id === selectedTestForBulkUpdate)?.name}
								</h3>
								<table className="min-w-full divide-y divide-gray-200">
									<thead className="bg-gray-50">
										<tr>
											...
											<tr key={service.id}>
												...
												<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
													<Input
														type="number"
														value={
															service.new_discounted_price !== undefined
																? service.new_discounted_price
												...
												<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
													<Input
														type="number"
														value={
															service.new_home_collection_fee !== undefined
																? service.new_home_collection_fee
																: service.home_collection_fee || ''
														}
														onChange={(e) =>
															handleBulkPriceChange(
																service.id,
																'new_home_collection_fee',
																parseFloat(e.target.value)
															)
														}
														className="w-24"
