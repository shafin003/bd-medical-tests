...
export async function GET(_request: NextRequest): Promise<NextResponse<TestCategory[] | { error: string }>> {
...
export async function POST(_request: NextRequest): Promise<NextResponse<TestCategory | { error: string }>> {
  const auth = await isAdmin(_request);
  if (!auth.authenticated) return auth.response;

  try {
    const newCategory: TestCategory = await request.json();
